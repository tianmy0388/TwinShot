...
game_finished = False
game_finished_label = Actor('game_finished')
game_finished_label.x = WIDTH // 2
game_finished_label.y = HEIGHT // 2 - 50
lever_clear_time = None

continue0 = Actor('restart_level0')
continue1 = Actor('restart_level1')
continue0.x = WIDTH // 2
...
tmx_data = load_pygame('images/Twin_shot_map3.tmx')
# 瓦片粉碎动画
tile_break = [Actor(f'smash{i}') for i in range(6)]
...
# 新增碎裂瓦片管理
smashed_tiles = {}  # {(tile_x, tile_y): {'time':触发时间, 'frame':动画帧, 'pending':True/False}}

SMASH_PENDING = 'pending'      # 等待1秒
SMASH_BREAKING = 'breaking'    # 播放粉碎动画0.5秒
SMASH_EMPTY = 'empty'          # 空白
SMASH_RESTORING = 'restoring'  # 恢复动画（可选）

SMASH_ANIM_DURATION = 0.5  # 动画05秒
SMASH_ANIM_FRAMES = 6
SMASH_ANIM_INTERVAL = SMASH_ANIM_DURATION / SMASH_ANIM_FRAMES  # 每帧持续时间
SMASH_PENDING_DURATION = 1.0   # 等待1秒
SMASH_TOTAL_DURATION = 3.0     # 总共3秒
SMASH_RESTORE_DURATION = 0.5  # 恢复动画时长
...
red_enemy_run_l = [Actor(f'red_e_l{i}') for i in range(10)]  # 红色敌人动画
red_enemy_run_r = [Actor(f'red_e_r{i}') for i in range(10)]
red_enemy_dead = [Actor(f'red_e_dead{i}') for i in range(4)]  # 红色敌人死亡动画

# fly敌人
fly_enemy_run_l = [Actor(f'fly_e_l{i}') for i in range(8)]
fly_enemy_run_r = [Actor(f'fly_e_r{i}') for i in range(8)]
fly_enemy_dead = [Actor(f'fly_e_dead{i}') for i in range(4)]  # 飞行敌人死亡动画
...
class Enemy:
    ...
    def update(self):
        if self.state == 'alive':
            ...
            foot_y = self.y + enemy_run_r[0].height // 2 - 4
            ...
            if is_solid_at(check_x, foot_y):
                ...
                # 悬崖检测：前方脚下是否有solid
                cliff_check_x = check_x
                cliff_check_y = foot_y + 8  # 脚下8像素
                if not is_solid_at(cliff_check_x, cliff_check_y):
                    self.vx = -self.vx
                    self.facing = 'l' if self.vx < 0 else 'r'
                else:
                    self.x = next_x
            ...
            if self.y + enemy_run_r[0].height // 2 < 0:
                ...
                self.y -= map_height_px  # 这里实现从顶部降落
            if self.x + 5 < 0:
                self.x += map_width_px
            elif self.x - 5 > map_width_px:
                self.x -= map_width_px
    ...
    def draw(self, screen, cam_x, cam_y):
        if self.state == 'alive':
            ...
            if self.invincible and (pygame.time.get_ticks() // 100) % 2 == 0:
                return  # 跳过绘制实现闪烁
            img = f'red_e_{self.facing}{self.anim_frame}'
        else:
            img = f'red_e_dead{self.anim_frame}'
        enemy_actor = Actor(img)
        enemy_actor.pos = (self.x - cam_x, self.y - cam_y)
        enemy_actor.draw()
# 飞行敌人类
class FlyEnemy(Enemy):
    def __init__(self, x, y):
        ...
        self.anim_interval = 6
        self.vx = ENEMY_SPEED
        self.dead_gravity = 0.2  # 死亡时的重力加速度
        self.dead_vy = 0 # 死亡时的垂直速度
        self.vy = 0.5
        self.float_range = 10 # 上下浮动范围
        self.base_y = y
        self.float_dir = 1
    ...
    def update(self):
        if self.state == 'alive':
            # 检查左右前方是否有solid瓦片
            next_x = self.x + self.vx
            foot_y = self.y + fly_enemy_run_r[0].height // 2 - 4
            check_x = next_x + (5 if self.vx > 0 else -5)
            if is_solid_at(check_x, foot_y):
                self.vx = -self.vx
                self.facing = 'l' if self.vx < 0 else 'r'
            else:
            ...
            # 上下浮动
            self.y += self.float_dir * self.vy
            if abs(self.y - self.base_y) > self.float_range:
                self.float_dir *= -1
            ...
            # 死亡状态下，应用重力
            self.vy += self.dead_gravity
            if self.vy > 7:  # 限制最大下落速度
                self.vy = 7
            self.y += self.vy
            self.anim_timer += 1
    ...
    def draw(self, screen, cam_x, cam_y):
        if self.state == 'alive':
            img = f'fly_e_{self.facing}{self.anim_frame}'
        else:
            img = f'fly_e_dead{self.anim_frame}'
        actor = Actor(img)
        actor.pos = (self.x - cam_x, self.y - cam_y)
        actor.draw()
# 敌人列表
# enemies = [
...
red_enemy_spawn_points = [
    (12, 3)
]
fly_enemy_spawn_points = [
    (10, 7)
]
enemies = []
for col, row in enemy_spawn_points:
    x = col * TILE_SIZE + TILE_SIZE // 2
    y = row * TILE_SIZE + TILE_SIZE // 2
    enemies.append(Enemy(x, y))
for col, row in red_enemy_spawn_points:
    x = col * TILE_SIZE + TILE_SIZE // 2
    y = row * TILE_SIZE + TILE_SIZE // 2
    enemies.append(RedEnemy(x, y))
for col, row in fly_enemy_spawn_points:
    x = col * TILE_SIZE + TILE_SIZE // 2
    y = row * TILE_SIZE + TILE_SIZE // 2
    enemies.append(FlyEnemy(x, y))
...
player.world_y = 12 * TILE_SIZE + TILE_SIZE // 2  # 第12行中心（行索引从0开始）
player.vx = 0
player.vy = 0
...
def is_solid_at(world_x, world_y, for_standing=False):
    ...
    # 检查是否是碎掉的smash瓦片
    # 只有pending状态下可站立
    if (tile_x, tile_y) in smashed_tiles:
        info = smashed_tiles[(tile_x, tile_y)]
        if info['state'] == SMASH_PENDING:
            return True
        else:
            return False
...
# # 判断是否可以穿透（passble）瓦片
def is_passble_at(world_x, world_y):
    tile_x = int(world_x // TILE_SIZE)
    tile_y = int(world_y // TILE_SIZE)
    tile_x_mod = tile_x % tmx_data.width
    tile_y_mod = tile_y % tmx_data.height
    for layer in tile_layers:
        ...
        if props is not None:
            # 默认为 True（可穿透），只有明确为 False 时才阻挡
            return props.get('passble', True)
    return True
...
def update_player():
    ...
    # 检查脚底多个点
    center_foot_x = player.world_x
    left_foot_x = player.world_x - player.width * 0.4
    right_foot_x = player.world_x + player.width * 0.4

    # 检查是否踩在smash瓦片上
    for foot_x in [left_foot_x, center_foot_x, right_foot_x]:
        tile_x = int(foot_x // TILE_SIZE)
        tile_y = int(foot_y // TILE_SIZE)
        tile_x_mod = tile_x % tmx_data.width
        tile_y_mod = tile_y % tmx_data.height
        for layer in tile_layers:
            gid = layer.data[tile_y_mod][tile_x_mod]
            if gid == 0:
                continue
            props = tmx_data.get_tile_properties_by_gid(gid)
            if props and props.get('smash') in (True, 'true'):
                if (tile_x, tile_y) not in smashed_tiles:
                    smashed_tiles[(tile_x, tile_y)] = {
                        'state': SMASH_PENDING,
                        'time': time.time(),
                        'frame': 0
                    }
    ...
    # 检查多个点是否在solid上
    left_on_solid = is_solid_at(left_foot_x, foot_y, for_standing=True)
    center_on_solid = is_solid_at(center_foot_x, foot_y, for_standing=True)
    right_on_solid = is_solid_at(right_foot_x, foot_y, for_standing=True)

    # 需要至少有中心点和另一个点在solid上，或者左右脚都在solid上
    on_solid_platform = (center_on_solid and (left_on_solid or right_on_solid)) or (left_on_solid and right_on_solid)

    if player.vy > 0 and on_solid_platform:
        # 检查是否站在箭上
        standing_on_arrow = False
        for arrow in arrows:
            if arrow.state == 'embedded':
                if (arrow.x - arrow.width / 2 <= center_foot_x <= arrow.x + arrow.width / 2 and
    ...
    # 处理头顶碰撞
    if player.vy < 0:
        head_x = player.world_x
        head_y = player.world_y - player.height / 2 - 1
        if not is_passble_at(head_x, head_y):
            # 头顶撞到不可穿透瓦片，vy清零并开始下落
            player.world_y = (int(head_y // TILE_SIZE) + 1) * TILE_SIZE + player.height / 2
            player.vy = 0
    ...
    for layer in tmx_data.visible_layers:
        if hasattr(layer, 'tiles'):
            for x, y, image in layer.tiles():
                info = smashed_tiles.get((x, y))
                if info:
                    if info['state'] == SMASH_PENDING:
                        screen.surface.blit(image, (x * TILE_SIZE - cam_x, y * TILE_SIZE - cam_y))
                    elif info['state'] == SMASH_BREAKING:
                        frame = info['frame']
                        anim_img = tile_break[frame]
                        # 顶端对齐
                        screen.surface.blit(anim_img._surf, (x * TILE_SIZE - cam_x, y * TILE_SIZE - cam_y))
                    elif info['state'] == SMASH_EMPTY:
                        pass  # 空白不绘制
                    # 可选：恢复动画
                    elif info['state'] == SMASH_RESTORING:
                        restore_elapsed = time.time() - info['time']
                        alpha = min(255, int(255 * (restore_elapsed / SMASH_RESTORE_DURATION)))
                        # 创建一个带 alpha 通道的临时 surface
                        temp = pygame.Surface(image.get_size(), pygame.SRCALPHA)
                        temp.blit(image, (0, 0))
                        temp.set_alpha(alpha)
                        screen.surface.blit(temp, (x * TILE_SIZE - cam_x, y * TILE_SIZE - cam_y))
                else:
                    screen.surface.blit(image, (x * TILE_SIZE - cam_x, y * TILE_SIZE - cam_y))
...
def draw(screen):
    ...
    if game_finished:  # 如果关卡已完成
        # 半透明遮罩
        s = pygame.Surface((WIDTH, HEIGHT), pygame.SRCALPHA)
        s.fill((0, 0, 0, 128))
        screen.surface.blit(s, (0, 0))
        # 绘制关卡完成标签
        game_finished_label.draw()
...
def update_enemies():
    ...
    enemy_num = sum(1 for e in enemies if e.state == 'alive')
                    ...
                    if isinstance(enemy, RedEnemy):
                        if not enemy.invincible:
                            ...
                            if enemy.life <= 0:
                                ...
                                enemy.invincible_timer = 180  # 3秒
                        arrow.state = 'enemy_hit'
                        arrow.vx = 0
                    else:
                        enemy.state = 'dead'
                        enemy_num -= 1
...
def update():
    ...
    每帧自动调用：
      ...
      3. 按 ESC 返回关卡选择
    """
    global camera_x, camera_y, level_clear_time, game_finished, is_pause
    global enemy_num, player_dead, life_num, player_death_anim, player_death_vy, player_death_timer
    if is_pause or game_finished:
    ...
    # 更新 smashed_tiles 中的瓦片状态
    now = time.time()
    to_restore = []
    for key, info in smashed_tiles.items():
        elapsed = now - info['time']
        if info['state'] == SMASH_PENDING:
            if elapsed >= SMASH_PENDING_DURATION:
                info['state'] = SMASH_BREAKING
                info['time'] = now
                info['frame'] = 0
        elif info['state'] == SMASH_BREAKING:
            if elapsed < SMASH_ANIM_DURATION:
                frame = min(int(elapsed // SMASH_ANIM_INTERVAL), SMASH_ANIM_FRAMES - 1)
                info['frame'] = frame
            else:
                info['state'] = SMASH_EMPTY
                info['time'] = now
        elif info['state'] == SMASH_EMPTY:
            if elapsed >= SMASH_TOTAL_DURATION - SMASH_PENDING_DURATION - SMASH_ANIM_DURATION:
                # 可选：进入恢复动画
                info['state'] = SMASH_RESTORING
                info['time'] = now
        # 可选：恢复动画
        elif info['state'] == SMASH_RESTORING:
            # ...恢复动画逻辑...
            restore_elapsed = now - info['time']
            if restore_elapsed >= SMASH_RESTORE_DURATION:
                to_restore.append(key)
    for key in to_restore:
        smashed_tiles.pop(key)

    # 检查是否清除所有敌人
    if enemy_num == 0 and not game_finished:
...
def on_mouse_down(pos, switch_state):
    """处理鼠标点击事件"""
    global is_pause, game_finished, player_dead, player_death_anim
    if not is_pause:
        if pause1.collidepoint(pos):
            is_pause = True
    else:
...
def reset_level():
    global player, enemies, arrows, camera_x, camera_y, camera_target_x, camera_target_y, is_pause, game_finished, level_clear_time, enemy_num, life_num
    global player_dead, player_death_anim, player_death_vy, player_death_timer
    # 重新初始化玩家
    player.world_x = 2 * TILE_SIZE + TILE_SIZE // 2
    player.world_y = 12 * TILE_SIZE + TILE_SIZE // 2
    player.vx = 0
    player.vy = 0
    player.on_ground = False
    player.anim_frame = 0
